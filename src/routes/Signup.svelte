<script>
  ...
  const handlePassword = () => {
    if (password.length >= 6) passwordColor = "yellow";
    if (password.length >= 4 && passwordSymbol()) passwordColor = "yellow";
    if (password.length >= 10 && passwordSymbol()) passwordColor = "green";
    if (password.length >= 40) passwordColor = "green";
    if (password.length < 6) passwordColor = "red";
  };
...
<div class="flex justify-center w-screen">
  <div
    ...
    aria-live="polite"
  >
    <p class="font-bold text-2xl m-5">Sign up</p>
    <form class="form-widget" on:submit|preventDefault={handleLogin}>
      <div>
        ...
        />
        <div class="w-full bg-{passwordColor}-400 h-2 mt-2 rounded-lg" />
      </div>
    ...
    <a class="m-5 hover:text-gray-300 duration-300" href="#/account">Sign in</a>
  </div>
</div>
